...
class proto_server_setup:
    ...
    def Load(cls, data: bytes) -> "proto_server_setup":
        return psi.proto_server_setup.Load(data)
...
class proto_request:
    ...
    def Load(cls, data: bytes) -> "proto_request":
        return psi.proto_request.Load(data)
...
class client:
    def __init__(self, reveal_intersection: bool, key_bytes: bytes = None):
        ...
        """
        if key_bytes:
            self.data = psi.client.CreateFromKey(key_bytes, reveal_intersection)
        else:
            self.data = psi.client.CreateWithNewKey(reveal_intersection)
    ...
    def CreateFromKey(cls, key_bytes: bytes, reveal_intersection: bool):
        return psi.client.CreateFromKey(key_bytes, reveal_intersection)
...
class server:
    def __init__(self, reveal_intersection: bool, key_bytes: bytes = None):
        ...
        """
        if key_bytes:
            self.data = psi.server.CreateFromKey(key_bytes, reveal_intersection)
        else:
            self.data = psi.server.CreateWithNewKey(reveal_intersection)
    ...
    @classmethod
    def CreateFromKey(cls, key_bytes: bytes, reveal_intersection: bool):
        return psi.server.CreateFromKey(key_bytes, reveal_intersection)
