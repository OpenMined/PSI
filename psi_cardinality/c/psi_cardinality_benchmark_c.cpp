...
void BM_ServerSetup(benchmark::State &state, double fpr) {
  ...
  for (int i = 0; i < num_inputs; i++) {
    ...
    inputs[i] = {orig_inputs[i].c_str(), uint32_t(orig_inputs[i].size())};
  }
  std::string setup;
  int64_t elements_processed = 0;
  for (auto _ : state) {
    char *server_setup = nullptr;
    uint32_t server_setup_buff_len = 0;
...
void BM_ClientCreateRequest(benchmark::State &state) {
  ...
  for (int i = 0; i < num_inputs; i++) {
    ...
    inputs[i] = {inputs_orig[i].c_str(), uint32_t(inputs_orig[i].size())};
  }
  std::string request;
  int64_t elements_processed = 0;
  for (auto _ : state) {
...
void BM_ClientProcessResponse(benchmark::State &state) {
  ...
  for (int i = 0; i < num_inputs; i++) {
    inputs_orig[i] = absl::StrCat("Element", i);
    srv_inputs[i] = {inputs_orig[i].c_str(), uint32_t(inputs_orig[i].size())};
    cl_inputs[i] = {inputs_orig[i].c_str(), uint32_t(inputs_orig[i].size())};
  }
